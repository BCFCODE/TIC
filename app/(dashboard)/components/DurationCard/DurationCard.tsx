...
export default function DurationCard({ days, hours, minutes }: DurationCardProps) {
  return (
    <Card
      sx={{
        display: "flex",
    ...
    >
      ...
      >
        <Gauge size={50} color="#6b7280" />
      </Box>

      {/* Time Boxes */}
      <Box sx={{ display: "flex", gap: 1, ml: 3 }}>
        {[{ value: days, label: "DAYS" }, { value: hours, label: "HOURS" }, { value: minutes, label: "MINUTES" }].map(
          (time, index) => (
            <Box
              key={index}
              sx={{
